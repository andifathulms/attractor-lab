...
**One known deviation from the rest of this document:** the Next config file is `next.config.mjs`, not `next.config.js` — the "Commands" section above should be read with that in mind. The "Layout" tree's single `components/panel/` and `components/readout/` is now accurate in spirit though not in filename: `components/readout/ReadoutStrip.tsx` and `components/panel/PanelShell.tsx` are the shared shells (`ReadoutStrip` takes a `fields`/`tags` list; `PanelShell` owns the collapse-to-edge-tab chrome), and each route keeps a thin same-named wrapper (`ComparisonReadoutStrip`, `SectionReadoutStrip`, `BifurcationReadoutStrip`; `ComparisonPanel`, `SectionPanel`, `BifurcationPanel`) that supplies its own fields or form contents — `jelajah`'s `ControlPanel` and the base `ReadoutStrip` call the shells directly.

`pnpm test:analytic` and `pnpm test:order` are green. Trail colours are centralised in `lib/render/trail-colors.ts`, keyed by role, with `tests/design/no-raw-trail-colors.test.ts` failing the build on any `rgba(`/`rgb(` literal reintroduced under `components/`. `banding` has its own separation plot (`components/comparison/ComparisonSeparationPlot.tsx`, fed by `workers/compare.worker.ts`'s per-step `eulerVsRk4`/`rk2VsRk4` series) docked above its readout strip, same log-axis primitive (`lib/render/separation-plot.ts`) as `jelajah`'s divergence pair — curves told apart by dash pattern, never hue, per invariant 8. Clicking "check convergence" swaps that plot to a static before/after view — the same two curves at `dt` and `dt/2` (`lib/convergence-series.ts`, outside `lib/dynamics`) — so halving the step's effect on the error is a visible gap, not just the numeric rows `ComparisonPanel` still shows underneath. `DESIGN-REWORK.md` §2's canvas/readout-strip/panel unification is done. `/sistem` (§3) now renders a thumbnail grid instead of a text list: flow systems get an SVG path precomputed at build time (`lib/render/thumbnail.ts`, using a deliberately asymmetric initial state — see its comment on why a symmetric one collapses Thomas and Halvorsen to a fixed point) with their integrator and step size captioned per invariant 4; map systems reuse `MapPreview` at a smaller size. Next per `DESIGN-REWORK.md`'s build order: `VerifiedConstants` tolerance strips (§4), and a shared error surface for worker failure and boundedness-guard trips (§5).
